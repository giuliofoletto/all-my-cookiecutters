...
def process_existing_file_name(file_name, folders = [], formats=[]):
    ...
    for folder in folders:
        base_candidates.append(Path(__file__).parent.parent.absolute() / folder / file_name)
    candidates = []
...
def save_figures(figures, path=None, prefix=None):
    path, prefix, prefix_sep = process_default_path_prefix_input(path, prefix)
    for label, figure in figures.items():
        figure.savefig(path / (prefix + prefix_sep + label + ".png"))


def save_report(dictionary, path=None, prefix=None):
    path, prefix, prefix_sep = process_default_path_prefix_input(path, prefix)
    file_path = path / (prefix + prefix_sep + "report.json")
